...
echo -e "  --- ${CYAN}Qwen3 Family (Latest Generation)${RESET} ---"
echo -e "  ${YELLOW}[ 6]${RESET} Qwen3 (8B)               ${CYAN}[Text]${RESET}         ${DIM}(~5.2 GB)${RESET}   ${GREEN}[STANDARD]${RESET} ${MAGENTA}- FAST${RESET}"
echo -e "  ${YELLOW}[ 7]${RESET} Qwen3 (14B)              ${CYAN}[Text]${RESET}         ${DIM}(~9.3 GB)${RESET}   ${GREEN}[GPU]${RESET}      ${MAGENTA}- HIGH QUALITY${RESET}"
echo -e "  --- ${CYAN}High-VRAM GPU Tier (16 GB+ VRAM)${RESET} ---"
echo -e "  ${YELLOW}[ 8]${RESET} Qwen2.5-Coder (32B Q4)   ${CYAN}[Code]${RESET}         ${DIM}(~19 GB)${RESET}    ${GREEN}[GPU]${RESET}      ${MAGENTA}- BEST CODE${RESET}"
echo -e "  ${YELLOW}[ 9]${RESET} DeepSeek-R1 (32B Q4)     ${CYAN}[Reasoning]${RESET}    ${DIM}(~19 GB)${RESET}    ${GREEN}[GPU]${RESET}      ${MAGENTA}- DEEP REASON${RESET}"
echo -e "  ${YELLOW}[10]${RESET} Qwen2.5 (32B Q4)         ${CYAN}[Text]${RESET}         ${DIM}(~19 GB)${RESET}    ${GREEN}[GPU]${RESET}      ${MAGENTA}- MOST CAPABLE${RESET}"
...
if [ -d "$MANIFEST_DIR" ]; then
    PRESET_SKIP="gemma-4-e2b-it-q4_k_m-local|gemma-4-e2b-it-q6_k-local|gemma-4-e4b-it-q4_k_m-local|qwen3.5|ministral-3|qwen3|qwen2.5-coder|deepseek-r1|qwen2.5"
...
for T in "${TOKENS[@]}"; do
    ...
    elif [ "$T" = "6" ]; then SELECTED_MODELS+=("$T"); SELECTED_NAMES+=("Qwen3 (8B)"); SELECTED_TAGS+=("qwen3:8b")
    elif [ "$T" = "7" ]; then SELECTED_MODELS+=("$T"); SELECTED_NAMES+=("Qwen3 (14B)"); SELECTED_TAGS+=("qwen3:14b")
    elif [ "$T" = "8" ]; then SELECTED_MODELS+=("$T"); SELECTED_NAMES+=("Qwen2.5-Coder (32B Q4)"); SELECTED_TAGS+=("qwen2.5-coder:32b")
    elif [ "$T" = "9" ]; then SELECTED_MODELS+=("$T"); SELECTED_NAMES+=("DeepSeek-R1 (32B Q4)"); SELECTED_TAGS+=("deepseek-r1:32b")
    elif [ "$T" = "10" ]; then SELECTED_MODELS+=("$T"); SELECTED_NAMES+=("Qwen2.5 (32B Q4)"); SELECTED_TAGS+=("qwen2.5:32b")
    else
